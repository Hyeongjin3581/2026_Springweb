...
async function boardFindAll( ){ console.log( "boardFindAall 열림");
    ...
    const 대기명단리스트 = 응답결과.data;
        for( let index = 0 ; index <= 대기명단리스트.length-1 ; index++ ){
            const 대기명단객체 = 대기명단리스트[ index ]; 
            // `백틱이란? 문자열과 문자열 사이에 ${} 이용하여 변수 대입가능
            html += `<tr>
                    <td> ${ 대기명단객체.no } </td> <td> ${ 대기명단객체.phone } </td> 
                    <td> ${ 대기명단객체.people } </td> 
                    <td> 
                        <button onclick="boardUpdate('${ 대기명단객체.phone }')">수정</button> 
                        <button onclick="boardDelete('${ 대기명단객체.phone }')">삭제</button> 
                    </td> 
                    </tr>`
        } // for end 
...
async function boardSave(){ // 동기화함수 async
    ...
    const response = await axios.post( `/board/save?phone=${ phone }&people=${ people }` );
    // 3. 결과출력 
...
// [3] 수정
async function boardUpdate( phone ){
    // 1. 수정할 내용 입력받기 prompt
    const people = prompt('수정할 일행 수: ');
    // 2. 수정처리 : axios 이용하여 백엔드에게 수정 요청/응답
    const response = await axios.put( `/board/update?phone=${ phone }&people=${ people }`);
    // 3. 결과
    if( response.data == true ){
        alert('수정 성공');
        boardFindAll();
...
// [4] 삭제
async function boardDelete( phone ){ // async( axios동기화 )
    // no : 삭제할 게시물번호 / 클릭한 게시물번호
    // 1. 삭제처리 : axios 이용하여 백엔드에게 삭제 요청/응답
    const response = await axios.delete( `/board/delete?phone=${ phone }` );
    // 2. 결과
    if( response.data == true ){
        alert('삭제 성공');
        boardFindAll();
