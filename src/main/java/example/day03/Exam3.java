...
public class Exam3 {
 ...
 // [ 1 ] 전통 방식의 객체 생성 ( 강한결합 ) 계속적인 dao 인스턴스 생성가능
 ...
 class SampleController2{
    void method(){
        ...
        SampleDao2 sampleDao = SampleDao2.getInstance();
    }
}
 ...
 class SampleController3{
        @Autowired // 의존성주입 ( 스프링 컨테이너에 등록된 빈(객체)
...
/*
    즉 ] 다른 클래스의 메소드 호출하기 위해서는 객체가 필요.
    1. 전통) new 클래스명().메소드명();
    ...
    @Componet  등록되는 시점 : @SpringBootApplication(@ComponetScan)
    즉] 스프링이 켜지면서 폴더내 @ Componet 다 찾아(같은폴더/하위폴더) 내서 스프링 컨테이너 등록 구조
    AppStart 주의할점 : 최상위에 등록한다. 
